...
# -----------------------------------------------------------------------------
# NOTE:
# This is essentially the same as model-2 but with area covariate removed and
# spelling replacing letter sound knowledge.
# -----------------------------------------------------------------------------

"""
Model 3 (RI-CLPM with Beta-Binomial outcomes)
=============================================
...
Dims:
    person: N
    construct: K = 3 ("Vocab","Spelling","WordReading")
    time: T = 4
"""
...
J_vocab = 30  # ?? receptive vocabulary items
J_sp = 25  # ?? spelling items
J_wr = 105  # ?? word reading items
...
class ModelSpecification:
    ...
    sp : (N, 4) int
        Spelling scores over 4 time points
    wr : (N, 4) int
    ...
    def __init__(
        ...
        for name, arr in [("rv", rv), ("sp", sp), ("wr", wr)]:
        ...
        self.sp = np.ascontiguousarray(sp.astype(np.float64, copy=True))
...
# This function:
#   1) stacks rv/sp/wr -> y_float with shape (N, K, T),
...
def prepare_data(spec: ModelSpecification, cfg: ModelConfig) -> PreparedData:
    ...
    """
    N = spec.age.shape[0]
    constructs = np.array(["Vocab", "Spelling", "WordReading"], dtype=str)
    K = constructs.size
    T = spec.rv.shape[1]

    # Stack outcomes to (N,K,T); allow NaN for missing
    y_float = np.stack([spec.rv, spec.sp, spec.wr], axis=1).astype(np.float64)
    ...
    n_ct = np.array(
        [[cfg.J_vocab] * T, [cfg.J_sp] * T, [cfg.J_wr] * T], dtype=np.int64
    )  # (K,T)
    ...
    X = np.column_stack([gender_c]).astype(np.float64)
    covar_names = ["gender"]
...
def build_model(
    ...
    # Order must match `constructs` ("Vocab","Spelling","WordReading")
    # Note: not strictly priors - from study, but could be estimated from other studies
    mu0_prob = np.array([0.32, 0.08, 0.16], dtype=np.float64)
